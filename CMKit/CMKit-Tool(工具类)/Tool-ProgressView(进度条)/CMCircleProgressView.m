...
#define CMFreshGreen    [UIColor colorWithRed:77.0 / 255.0 green:196.0 / 255.0 blue:122.0 / 255.0 alpha:1.0f]
  ...
  overrideLineWidth:(NSNumber *)overrideLineWidth
{
    ...
    if (self) {
        _total = total;
        _current = current;
        _strokeColor = CMFreshGreen;
        _duration = 1.0;
        _chartType = CMChartFormatTypePercent;
...
- (void)strokeChart
{
    ...
    if (_displayCountingLabel) {
        ...
        switch (self.chartType) {
            case CMChartFormatTypePercent:
                format = @"%d%%";
                break;
            case CMChartFormatTypeDollar:
                format = @"$%d";
                break;
            case CMChartFormatTypeDecimal:
                format = @"%.1f";
                break;
            case CMChartFormatTypeDecimalTwoPlaces:
                format = @"%.2f";
                break;
            case CMChartFormatTypeNone:
            default:
                format = @"%d";
                break;
        }
