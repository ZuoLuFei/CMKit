    ...
    for (NSInteger i = 0; i<_valueArr.count; i++) {
        ...
        for (NSInteger j = 0; j<arr.count; j++) {
            ...
            [UIView animateWithDuration:1 animations:^{
                 ...
                 itemsView.frame = CGRectMake((i * arr.count + j)*self->_columnWidth + i*self->_typeSpace+self->_originSize.x + self->_typeSpace, CGRectGetHeight(self.frame) - height - self->_originSize.y -1, self->_columnWidth, height);
                ...
                if (finished) {
                    ...
                    CGSize size = [str boundingRectWithSize:CGSizeMake(self->_columnWidth, MAXFLOAT) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:9]} context:nil].size;
                    
                    textLayer.frame = CGRectMake((i * arr.count + j)*self->_columnWidth + i*self->_typeSpace+self->_originSize.x + self->_typeSpace, CGRectGetHeight(self.frame) - height - self->_originSize.y -3 - size.height, self->_columnWidth, size.height);
                    ...
                    [self->_BGScrollView.layer addSublayer:textLayer];
                    ...
                    if (i==self->_valueArr.count - 1&&j == arr.count-1 && self->_isShowLineChart) {
                        ...
                        for (int32_t m=0;m<self->_lineValueArray.count;m++) {
                            NSLog(@"%@",self->_drawLineValue[m]);
                            if (m==0) {
