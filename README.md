...
整个工具类[具体地址](https://github.com/ZuoLuFei/CMKit)：pod 'CMKit', '~> 0.1.3'

    分类：pod 'CMKit/Category', '~> 0.1.3'

    警告框：pod 'CMKit/AlertView', '~> 0.1.3’

    动画效果：pod 'CMKit/AnimationEffect', '~> 0.1.3’

    HUD提示框：pod 'CMKit/ProgressHUD', '~> 0.1.3’

    二维码：pod 'CMKit/QRCode', '~> 0.1.3’

    新特性：pod 'CMKit/NewFeatures', '~> 0.1.3’  (目前使用传入图片数组即可，后期会不断完善)

    公用父类：pod 'CMKit/CommonSuperClass', '~> 0.1.3’  (目前暂时只整理CMBaseViewController和CMBaseNavigationController，根据使用若有补充之处，请及时提出)

    相机、相册：pod 'CMKit/ImagePicker', '~> 0.1.3’    打开系统自带相册选择和相机，多选图片请移步下面'图片选择器'框架

    城市列表选择：pod 'CMKit/CityList', '~> 0.1.3’

    FMDB操作：pod 'CMKit/FMDBOperation', '~> 0.1.3’
